...
# encoding: utf-8
## 匿名login
from flask import Flask, Blueprint, session,request,render_template,jsonify
...
@auth.route('/login-bak', methods=['GET', 'POST'])
def login_bak():
    print("$$$$", current_user)
...
@auth.route('/login2-bak', methods=['GET', 'POST'])
def signin():
...
@app.route('/api/v1.0/user_login', methods=['PUT'])
def login_api():
    #req=request.json
    #print('#####',req)
    u = request.form.get('name')
